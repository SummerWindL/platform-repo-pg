...
public interface UserAccountPrjRepository extends CommonCustomRepository<BasePlpgsqlModel, Integer> {
    ...
    @Query(value= "select * from aimb.f_mb_user_account_prj_query(?1)", nativeQuery = true)
    BasePlpgsqlModel queryUserAccountPrj(String in_userid);

    @Query(value= "select * from aimb.f_mb_user_account_prj_save(?1, ?2, ?3)", nativeQuery = true)
